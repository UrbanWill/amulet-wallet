...
import { KeyringOption } from "./options/index";
...
export class Keyring extends Base implements KeyringStruct {
  ...
  public addExternal(
    ...
    const pair = this.keyring.addFromAddress(
      address,
      objectSpread<KeyringJson$Meta>({}, meta, { isExternal: true }),
      null
    );
  ...
  public addHardware(
    ...
    return this.addExternal(
      address,
      objectSpread<KeyringPair$Meta>({}, meta, {
        hardwareType,
        isHardware: true,
      })
    );
  ...
  public addMultisig(
    ...
    const who = u8aSorted(addresses.map((who) => this.decodeAddress(who))).map(
      (who) => this.encodeAddress(who)
    );
  ...
  public async backupAccounts(
    addresses: string[],
    password: string
  ): Promise<KeyringPairs$Json> {
    ...
    return objectSpread(
      {},
      jsonEncrypt(
        stringToU8a(JSON.stringify(accounts)),
        ["batch-pkcs8"],
        password
      ),
      {
  ...
  public getAccounts(): KeyringAddress[] {
    ...
    return Object.keys(available)
      ...
      .filter(
        (account): account is KeyringAddress =>
          !!account && (env.isDevelopment() || account.meta.isTesting !== true)
      );
  }
  ...
  private loadContract(json: KeyringJson, key: string): void {
    ...
    json.meta.genesisHash =
      json.meta.genesisHash || json.meta.contract?.genesisHash;
  ...
  private allowGenesis(
    json?: KeyringJson | { meta: KeyringJson$Meta } | null
  ): boolean {
    if (json?.meta && this.genesisHash) {
      const hashes: (string | null | undefined)[] = Object.values(chains).find(
        (hashes): boolean => hashes.includes(this.genesisHash || "")
      ) || [this.genesisHash];

      if (json.meta.genesisHash) {
        return (
          hashes.includes(json.meta.genesisHash) ||
          this.genesisHashes.includes(json.meta.genesisHash)
        );
