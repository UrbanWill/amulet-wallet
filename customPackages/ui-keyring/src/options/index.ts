...
import { obervableAll } from "../observable/index";
...
export class KeyringOption implements KeyringOptionInstance {
  ...
  public readonly optionsSubject = new BehaviorSubject<KeyringOptions>(
    this.emptyOptions()
  );
  ...
  public init(keyring: KeyringStruct): void {
    ...
    this.#allSub = obervableAll.subscribe((): void => {
      ...
      opts.all = ([] as KeyringSectionOptions).concat(
        opts.account,
        opts.address
      );
      opts.allPlus = ([] as KeyringSectionOptions).concat(
        opts.account,
        opts.address,
        opts.contract
      );
  ...
  private linkItems(
    items: Record<string, KeyringSectionOptions>
  ): KeyringSectionOptions {
    return Object.keys(items).reduce(
      (result, header): KeyringSectionOptions => {
        const options = items[header];

        return result.concat(
          options.length ? [this.createOptionHeader(header)] : [],
          options
        );
      },
